...
{
    public partial class Inventory_Order_Form : Form
    {
        ...
        OracleCommand cmd;
        int IdNum;
        public Inventory_Order_Form()
        {
            InitializeComponent();
        }
        public Inventory_Order_Form(int number)
        {
            InitializeComponent();
            IdNum = number;
        }
        ...
        private void btn_Commit_Click(object sender, EventArgs e)
        {
            ...
            string Toti = textBasic.Text; // 텍스트 박스에 입력되는 값
            ...
            if (Cb_Basic.SelectedIndex == 0) // 또띠아
            {
                ...
                {
                    cmd.CommandText = ($"UPDATE Inventory set Toti = Toti + {Toti} where Branch_Id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Basic.SelectedIndex == 1) // 밥
            {
                if (textBasic.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Rice = Rice + {Rice} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Basic.SelectedIndex == 2) // 야채
            {
                if (textBasic.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Veg = Veg + {Veg} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            if (Cb_Main.SelectedIndex == 0) // 닭
            {
                if (textMain.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Chi = Chi + {Chi} where Branch_Id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Main.SelectedIndex == 1) // 소
            {
                if (textMain.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Cow = Cow + {Cow} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Main.SelectedIndex == 2) // 텐더
            {
                if (textMain.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Ten = Ten + {Ten} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            if (Cb_Sauce.SelectedIndex == 0) // 순한맛
            {
                if (textSauce.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set MildS = MildS + {MildS} where Branch_Id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Sauce.SelectedIndex == 1) // 보통맛
            {
                if (textSauce.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set NomS = NomS + {NomS} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Sauce.SelectedIndex == 2) // 매운맛
            {
                if (textSauce.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set HotS = HotS + {HotS} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            if (Cb_Any.SelectedIndex == 0) // 감자튀김
            {
                if (textAny.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set PotPlus = PotPlus + {PotPlus} where Branch_Id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Any.SelectedIndex == 1) // 감자추가
            {
                if (textAny.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Pot = Pot + {Pot} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Any.SelectedIndex == 2) // 소세지
            {
                if (textAny.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set  Sausage = Sausage + {Sausage} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Any.SelectedIndex == 3) // 치즈
            {
                if (textAny.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Che = Che + {Che} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }
            if (Cb_Any.SelectedIndex == 4) // 콜라
            {
                if (textAny.Text != "")
                {
                    cmd.CommandText = ($"UPDATE Inventory set Cola = Cola + {Cola} where branch_id = {IdNum}");
                    cmd.ExecuteNonQuery();
                }
            }

            MessageBox.Show("재고가 주문되었습니다.");
            Jaego_Clear();
            this.Close();
        }
        ...
        private void Inventory_Order_Form_Load(object sender, EventArgs e)
        {
